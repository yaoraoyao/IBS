$(function(){
    $("#dg").datagrid({
        ...
        columns:[[     
            ...
            {field:"descs",title:'descs',width:100,align:"center"},
            {field:"sn",title:'sn',width:100,align:"center"},
            {field:"menuId",title:'menuId',width:100,align:"center"},
        ]],
        toolbar: "#toolbar"
    });
    //实现翻页功能
...
window.methods = {
    ...
    edit:function (){
        ...
        var row = rows[0];
        $.getJSON("/permission/findOne.do",{id:row.id},function(data){
            //表单回填
            $("#ffff").form("load", data);
            //回填一下部门下拉框
    ...
    remove:function (){
        ...
        $.messager.confirm('确认','您确认想要删除记录吗？',function(r){
            if (r){
                ...
                $.getJSON("/permission/delete.do",{ids:ids.join(",")},function(data){
                    if(data.status == 200){
    ...
    submit:function (){
        ...
        $.postJSON("/permission/save.do",params,function(data){
            if(data.status == 200){
                $.messager.alert('消息',data.msg,"info");
                //关闭模态窗口
